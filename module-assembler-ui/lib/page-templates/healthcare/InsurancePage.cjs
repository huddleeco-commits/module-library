/**
 * Healthcare Insurance & Billing Page Template
 */

function generateInsurancePage(fixture, options = {}) {
  const { business, theme, pages } = fixture;
  const colors = theme.colors;
  const homeData = pages?.home || {};
  const insuranceData = homeData.sections?.find(s => s.type === 'insurance');
  const plans = insuranceData?.items || ['Blue Cross Blue Shield', 'Aetna', 'Cigna', 'United Healthcare', 'Medicare', 'Medicaid'];
...
const INSURANCE_PLANS = ${JSON.stringify(plans, null, 2)};
      ...
      <section style={{ padding: '80px 0' }}>
        <div style={styles.container}>
          ...
          <div style={{
            ...
            <a href="tel:${business.phone || '5557894567'}" style={{
              display: 'inline-flex',
              alignItems: 'center',
              gap: '8px',
              ...
              <Phone size={18} />
              Call ${business.phone || '(555) 789-CARE'}
            </a>
          </div>
        </div>
      </section>
...
module.exports = { generateInsurancePage };
