        ...
        <div style={styles.menuList}>
          {menuItems.map((item, idx) => (
            <Link key={idx} to={item.path} style={styles.menuItem}>
              <item.icon size={22} color="#b8860b" />
...
const styles = {
  ...
  avatar: { width: '80px', height: '80px', borderRadius: '50%', background: '#b8860b', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '32px', fontWeight: '700', margin: '0 auto 16px' },
  name: { fontSize: '24px', fontWeight: '700', color: '#f1f5f9', marginBottom: '4px' },
  email: { fontSize: '14px', color: '#f1f5f9', opacity: 0.7 },
  menuList: { display: 'flex', flexDirection: 'column', gap: '12px' },
  menuItem: { display: 'flex', alignItems: 'center', gap: '16px', padding: '20px', background: '#0f172a', borderRadius: '16px', textDecoration: 'none', boxShadow: '0 4px 20px rgba(0,0,0,0.3)' },
  menuInfo: { flex: 1, display: 'flex', flexDirection: 'column' },
  menuLabel: { fontWeight: '600', color: '#f1f5f9', marginBottom: '2px' },
  menuDesc: { fontSize: '13px', color: '#f1f5f9', opacity: 0.6 }
};
